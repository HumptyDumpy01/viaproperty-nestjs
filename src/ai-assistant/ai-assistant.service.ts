...
function attachPrefix(generationFor: string, tags: string, tone: string) {
  return `
Context: You are Viaproperty AI Assistant to help the users generate some fancy text snippets.
...
tell the user he should only attach related tags to the generationFor field. Your response should be consistent, concise. Also ,check 
what tone the user wants the snippet to be generated. YOU should return a complete plain text snippet without extra questions or quotes, because
I would implement feature to just insert the text onto the corresponding field.
Restrictions for fields(in chars): Property Title(Min 5, Max 100), Descriptions(Min 5, Max 700)
User request: generationFor field: ${generationFor}, tags attached by user: ${tags}, expected tone: ${tone}
  `;
}
